...
@interface ANBannerVIew ()<UIScrollViewDelegate>
//scrollView
@property(nonatomic, strong) UIScrollView * scrollView;
//pageControl
@property(nonatomic, strong) ANPagecontrol *pageControl;

/** 定时器 */
@property (nonatomic, strong) NSTimer *timer;
@end

@implementation ANBannerVIew
{
    ///滚动的时间间隔
...
{
    //初始化scrollview
    _scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, KScreenWidth, view_HEIGHT)];
    ...
    if (_imagesArray.count == 1) {
        self.scrollView.contentSize = CGSizeMake(view_WIDTH , view_HEIGHT);
        ...
        [self setImageView:imageView atIndex:0];
    ...
    self.pageControl = [[ANPagecontrol alloc] initWithFrame:CGRectMake(0, view_HEIGHT - 30, view_WIDTH, 30)];
...
{
    imageView.image = [UIImage imageNamed:_imagesArray[index]];
    NSString * str = [_imagesArray[index] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];

    [imageView sd_setImageWithURL:[NSURL URLWithString:str] placeholderImage:[UIImage imageNamed:@"third_img_wuxiaoxi"]];
}
